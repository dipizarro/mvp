...
export default function SocialPlanetsSection({ data, profile, name }: SocialPlanetsProps) {
  ...
  const renderItem = (label: string, key: 'jupiter' | 'saturn', icon: string, delay: number, item?: PlanetData) => {
    if (!item) return null;
    ...
    return (
      <div 
        ...
        style={{ 
          ...
          boxShadow: isOpen ? '0 8px 32px 0 rgba(255, 165, 0, 0.1)' : '0 4px 16px 0 rgba(0, 0, 0, 0.1)'
        }}
      >
        <button
          ...
          className={`w-full text-left px-6 py-4 font-bold text-lg transition-all duration-300 flex items-center gap-3 group ${
            isOpen 
              ? 'bg-gradient-to-r from-orange-600/20 to-yellow-600/20 border-b border-white/20' 
              : 'bg-gradient-to-r from-white/5 to-white/10 hover:from-orange-500/20 hover:to-yellow-500/20'
          }`}
        >
          <span className="text-2xl group-hover:scale-110 transition-transform duration-300">{icon}</span>
          <span className="flex-1">{label} en <span className="text-orange-300 font-extrabold">{item.sign}</span></span>
  ...
  return (
    <section id="sociales" className={`mb-12 space-y-6 transition-all duration-1000 ${
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={toggleAll}
          className="text-sm bg-gradient-to-r from-orange-600 to-yellow-600 hover:from-yellow-600 hover:to-orange-600 text-white px-4 py-2 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg"
        >
      ...
      <div className="space-y-4">
        {renderItem('♃ Júpiter', 'jupiter', '♃', 100, data.jupiter)}
        {renderItem('♄ Saturno', 'saturn', '♄', 200, data.saturn)}
      </div>
    </section>
  );
}
